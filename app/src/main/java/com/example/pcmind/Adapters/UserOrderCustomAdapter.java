...
public class UserOrderCustomAdapter extends ArrayAdapter<Product> {
    ...
    public UserOrderCustomAdapter(Context context, int resource, ArrayList<Product> items) {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (p != null) {
                ...
                ivDelete.setOnClickListener(view -> {
                    FavoriteRef =  database.getReference().child("Orders").child(User.getId());
                    FavoriteRef.child(p.getId()).removeValue().addOnCompleteListener(task ->{
                        if (task.isSuccessful())
                        {
                          this.remove(getItem(position));
                            Toast.makeText(mContext, "Product Removed From Order", Toast.LENGTH_SHORT).show();
                        }
                    });
