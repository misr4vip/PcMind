...
public class UserFavCustomAdapter extends ArrayAdapter<Product> {
    ...
    public UserFavCustomAdapter(Context context, int resource, ArrayList<Product> items) {
                    ...
                    FavoriteRef.child(p.getId()).removeValue().addOnCompleteListener(task ->{
                        if (task.isSuccessful())
                        {
                          this.remove(getItem(position));
                            Toast.makeText(mContext, "Product Removed From Favorite", Toast.LENGTH_SHORT).show();
                        }
                    });
